...
@section('title', config('simple-chat.titles.create', 'Start a Conversation'))
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:px-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900">
                    {!! config('simple-chat.titles.create', 'Start a Conversation') !!}
                </h3>
                @if(config('simple-chat.mode') === 'support')
                    <p class="mt-1 max-w-2xl text-sm text-gray-500">
                        You are about to start a new support conversation. A support agent
            ...
            <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
                <form action="{{ route('simple-chat.start') }}" method="POST" class="sm:p-6 p-4">
                    ...
                    @if(config('simple-chat.mode') !== 'support')
                        <div>
                            <label for="participant_id" class="block text-sm font-medium text-gray-700">User ID</label>
                            <div class="mt-1">
